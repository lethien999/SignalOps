...
export class UserService {
  ...
  async removeUserFromTenant(tenantId: Types.ObjectId, userId: Types.ObjectId): Promise<void> {
    ...
    Logger.info('User removed from tenant', { userId: userId.toString(), tenantId: tenantId.toString() });
  }

  async updateUserRole(tenantId: Types.ObjectId, userId: Types.ObjectId, roleId: string): Promise<UserDocument> {
    const user = await this.userModel.findById(userId);
    if (!user || user.tenantId.toString() !== tenantId.toString()) {
      throw new NotFoundException('User not found in this tenant');
    }
    ...
    Logger.info('User role updated', { userId: userId.toString(), tenantId: tenantId.toString(), roleId });

    return user;
  }

  async listTenantUsers(tenantId: Types.ObjectId, skip = 0, limit = 50): Promise<{ data: UserDocument[]; total: number }> {
