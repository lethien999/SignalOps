...
export type StatusEmissionPayload = Record<string, unknown>;
...
export class StatusGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  afterInit(server: Namespace) {
    server.on('error', (error: unknown) => {
      Logger.error('Socket.IO server error on /status namespace', error);
    });
  }

  handleConnection(client: Socket) {
    if (!this.isAuthorized(client)) {
      Logger.warn(`Unauthorized WebSocket client on /status: ${client.id}`);
    ...
    Logger.info(`WebSocket client connected to /status: ${client.id}`, {
      namespace: '/status',
  ...
  handleDisconnect(client: Socket) {
    ...
    Logger.info(`WebSocket client disconnected from /status: ${client.id}`, {
      namespace: '/status',
      totalClients: this.connectedClients.size,
    });
  }
  ...
  subscribe(@ConnectedSocket() client: Socket) {
    Logger.info(`Client subscribed to status: ${client.id}`);
    return { event: 'subscribed', namespace: '/status' };
  }

  /**
   * Broadcast queue depth to all connected clients
   */
  broadcastQueueDepth(payload: Record<string, unknown>): void {
    Logger.info('Broadcasting queue:depth to /status namespace', {
      depth: payload.depth,
      clientCount: this.connectedClients.size,
    });
    this.safeEmitToAll('queue:depth', payload);
  }

  /**
   * Broadcast worker statistics to all connected clients
   */
  broadcastWorkerStats(payload: Record<string, unknown>): void {
    Logger.info('Broadcasting worker:stats to /status namespace', {
      clientCount: this.connectedClients.size,
    });
    this.safeEmitToAll('worker:stats', payload);
  }
  ...
  private safeEmitToAll(event: string, payload: Record<string, unknown>): void {
    try {
      ...
      Logger.error(`Failed to emit ${event} on /status namespace`, error);
    }
  }
}
