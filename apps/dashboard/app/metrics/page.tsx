...
export default function MetricsPage() {
  ...
  const [slaSeverity, setSlaSeverity] = useState<
    'all' | 'low' | 'warning' | 'medium' | 'high' | 'critical'
  >('all');
  ...
  const costAlertTone = costMetrics?.warning
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Giám sát hiệu suất và phân tích dữ liệu theo thời gian thực.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard
          ...
          value={`${avgLatency}ms`}
          icon={TrendingUp}
          trend={avgLatency > 150 ? 'up' : 'stable'}
          trendValue={avgLatency > 150 ? 'Cao' : 'Bình thường'}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Theo dõi MTTR, Uptime và Alert Rate theo bộ lọc.
            </p>
          </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm mb-6">
        ...
        {!slaSnapshot || slaSnapshot.byDay.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart
              data={slaSnapshot.byDay.map((row) => ({ ...row, label: row.date.slice(5) }))}
            >
      ...
      <div className="grid gap-6 lg:grid-cols-2 mb-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 text-sm font-semibold uppercase tracking-wide text-gray-500">
            Signal Strength theo thời gian
          </h3>
          {chartData.length === 0 ? (
            <div className="flex h-64 items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50 text-sm text-gray-500">
              Chưa có dữ liệu
          ...
          {workerStats ? (
            <>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart
                  data={[
                    { name: 'Processing', value: workerStats.processing },
                    { name: 'Completed', value: workerStats.completed },
                    { name: 'Failed', value: workerStats.failed },
                  ]}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm mb-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Theo dõi chi phí hạ tầng, snapshot scale và cảnh báo khi vượt ngưỡng.
            </p>
          </div>
          <div
            className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium ${costAlertTone}`}
          >
            {costMetrics?.warning ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <MetricCard
            ...
            icon={TrendingUp}
            trend={
              scaleStatus?.recommendation === 'scale_up'
                ? 'up'
                : scaleStatus?.recommendation === 'scale_down'
                  ? 'down'
                  : 'stable'
            }
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <div>
            ...
            {costBreakdown.length === 0 ? (
              <div className="flex h-52 items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50 text-sm text-gray-500">
                Chưa có dữ liệu cost breakdown
              </div>
            ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <div className="h-3 rounded-full bg-gray-200">
              <div
                className="h-3 rounded-full bg-gradient-to-r from-purple-500 to-purple-400 transition-all"
                style={{ width: `${Math.min(100, (avgLatency / 300) * 100)}%` }}
              />
            </div>
          </div>
          <div>
            ...
            <div className="h-3 rounded-full bg-gray-200">
              <div
                className="h-3 rounded-full bg-gradient-to-r from-red-500 to-red-400 transition-all"
                style={{ width: `${Math.min(100, Number(avgPacketLoss) * 10)}%` }}
              />
            </div>
          </div>
          <div>
            ...
            <div className="h-3 rounded-full bg-gray-200">
              <div
                ...
                style={{ width: `${Math.min(100, (stats?.eventsPerMinute || 0) * 5)}%` }}
              />
            </div>
          </div>
        </div>
