...
export function AlertTable({ alerts, onSelectAlert, serverSide = false }: AlertTableProps) {
  ...
  >('all');
  const [statusFilter, setStatusFilter] = useState<'all' | 'open' | 'acknowledged' | 'resolved'>(
    'all'
  );
        ...
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-4 md:px-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">Lọc cảnh báo theo mức độ và trạng thái.</p>
              <button
                ...
                <div key={location}>
                  ...
                  {isExpanded &&
                    groupAlerts.map((alert) => {
                      const config =
                        severityConfig[String(alert.severity || '').toLowerCase()] ||
                        fallbackSeverityConfig;
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${config.badge}`}
                          >
